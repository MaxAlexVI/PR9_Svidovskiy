...
### Выводы
Пароли нельзя хранить в открытом виде, так как при утечке БД злоумышленники получат полный доступ ко всем аккаунтам в нашей системе + некоторые пользователи используют один пароль везде, следовательно взлом может произойти еще и в других системах.
Для обеспечения безопасности данных мы использовали алгоритм bcrypt (хеширование), так как он является достаточно надежным, имеет встроенную соль и просто реализуется. Используется именно хеширование пароля, а не шифрование, так как процесс шифрования обратим, а хеширование является односторонним преобразованием (получить пароль из хеша крайне затратно, если вообще возможно).
При неудачной попытке входа в систему мы выводи лишь общую ошибку (нет конкретики что именно введено неправильно), чтобы исключить вход при помощи брутфорса. 
